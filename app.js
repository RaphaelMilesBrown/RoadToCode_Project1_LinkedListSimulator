...
function drawList(providedList)
{
    ...
    let drawLocs = {drawBoxLoc: {x:20,y:20},drawTextLoc: {x:120,y:150}};
...
function clearCanvas(){canvasContext.clearRect(0,0,canvas.width,canvas.height);}
let thisList = new LinkedList();
function pushFrontFunction(providedList, pData){clearCanvas();providedList.pushFront(pData);drawList(providedList);}
function pushEndFunction(providedList, pData){clearCanvas();providedList.pushEnd(pData);drawList(providedList);}
function popFrontFunction(providedList){clearCanvas();providedList.popFront();drawList(providedList);}
function popEndFunction(providedList){clearCanvas();providedList.popEnd();drawList(providedList);}
document.getElementById("pushFrontButton").onclick = function(){pushFrontFunction(thisList, document.getElementById("pushFrontDataField").value)};
document.getElementById("pushEndButton").onclick = function(){pushEndFunction(thisList, document.getElementById("pushEndDataField").value)};
document.getElementById("popFrontButton").onclick = function(){popFrontFunction(thisList)};
document.getElementById("popEndButton").onclick = function(){popEndFunction(thisList)};
